<div class="social-footer" id="reloader">
    @foreach ($wall_replies as $wall_reply_details)
        @if ($wall_reply_details->user_type == 'admin')
            <div class="social-comment">
                <a href class="pull-left">
                    <img alt="image"
                        src="{{ asset('image/' . $wall_reply_details->user_admin->profile_picture) }}">
                </a>
                <div class="media-body">
                    <a href="#">
                        {{ $wall_reply_details->user_admin->name }}
                    </a>
                    {{ $wall_reply_details->content }}
                    <br>

                    <small
                        class="text-muted">{{ date('F j, Y', strtotime($wall_reply_details->created_at)) }}</small>
                </div>
            </div>
        @else
            <div class="social-comment">
